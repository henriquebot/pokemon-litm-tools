...
18. O payload entregue ao usuário deve ser JavaScript em texto puro. Não usar Base64, gzip, deflate ou outro transporte compactado/encodado.
19. Todo payload novo deve usar `tools/patch-runtime.cjs` e executar sua lógica dentro de `runPatch(...)`; falhas de runtime devem preservar o payload exato em `%TEMP%/pokemon-litm-last-failed.cjs` junto do SHA-256.
20. Escritas de patch devem passar por `writeNormalizedFiles(...)`, que normaliza LF e remove espaços/tabs no fim das linhas antes de gravar.
21. Pós-condições obrigatórias devem usar `assertNamedChecks(...)` com nomes individuais; é proibido responder apenas com uma mensagem genérica como “pós-condições falharam”.
22. `validateWorkingTree(...)` é o gate local reutilizável para `git diff --check`, lista exata de arquivos e `node --check` nos JS alterados.
23. O payload validado e o payload entregue devem ser byte a byte o mesmo texto; qualquer ajuste depois da validação invalida o gate e exige nova execução completa.
